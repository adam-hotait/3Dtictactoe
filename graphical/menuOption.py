...
class MenuOption(pygame.sprite.Sprite):
    ...
    def __init__(self):
        ...
        self.__text_position = self.position[0] + (self.size[0] - text_w) / 2, self.position[1] + (
                    self.size[1] - text_h) / 2
    ...
    def point_in_rectangle(self, point):
        ...
        if point[0] >= self.position[0] and point[1] >= self.position[1] and point[0] <= self.position[0] + self.size[
            0] and point[1] <= self.position[1] + self.size[1]:
            return True
        else:
            return False
